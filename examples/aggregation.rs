...
fn main() -> tantivy::Result<()> {
    ...
    let agg_res: AggregationResults = searcher.search(&AllQuery, &collector)?;
    let res: Value = serde_json::to_value(agg_res)?;
    ...
    // match serde_json::to_string_pretty(&expected_size_offset_json) {
    //     Ok(value) => print!("\n\n expected_size_offset_json => {}", value),
    //     Err(error) => eprintln!("Error: {}", error),
    // }
    //
    // match serde_json::to_string_pretty(&res_size_offset) {
    //     Ok(value) => print!("\n\n res_size_offset => {}", value),
    //     Err(error) => eprintln!("Error: {}", error),
    // }
